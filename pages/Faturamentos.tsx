...
import { exportToExcel, MONTHS, formatCurrency } from "@/lib/excel";
...
import {TrendingUp, Download, Plus, Search, Trash2, Upload} from "lucide-react";
...
export default function Faturamentos() {
  ...
  const { data, isLoading } = trpc.faturamentos.list.useQuery({ ano });

  const updateMut = trpc.faturamentos.update.useMutation({
    onSuccess: () => { utils.faturamentos.list.invalidate(); },
    onError: (e) => toast.error(e.message),
  });

  const upsertMut = trpc.faturamentos.upsert.useMutation({
    onSuccess: () => { utils.faturamentos.list.invalidate(); toast.success("Filial adicionada!"); setShowAdd(false); setNewRow({ filial: "", gerente: "", coordenador: "" }); },
    onError: (e) => toast.error(e.message),
  });

  const deleteMut = trpc.faturamentos.delete.useMutation({
    onSuccess: () => { utils.faturamentos.list.invalidate(); toast.success("Removido!"); setDeletingId(null); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(`Importação concluída! ${result.faturamentos?.upserted || 0} registros atualizados`);
  ...
  const columnTotals = useMemo(() => {
    ...
    MONTHS.forEach((m: any) => {
      totals[m.key] = filtered.reduce((s: any, r: any) => s + ((r as any)[m.key] || 0), 0);
    });
    totals.total = filtered.reduce((s: any, r: any) => s + (r.total || 0), 0);
    return totals;
  }, [filtered]);
  ...
  function handleExport() {
    const rows = filtered.map((r: any) => ({
      ...
      Total: r.total, Desafio: r.desafio,
    }));
    exportToExcel(rows, `Faturamentos_${ano}`, "Faturamentos");
  ...
  async function updateCell(id: number, field: string, value: string) {
    ...
    if (isOnline) {
      ...
      await offlineDb.faturamentos.update(id, { [field]: numVal, updatedAt: Date.now() });
      await enqueue("faturamentos", "update", id, payload);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Faturamentos</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Controle de faturamento por filial</p>
        </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-semibold">Faturamentos {ano}</CardTitle>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-3 py-3 font-semibold text-muted-foreground uppercase tracking-wider w-24">Gerente</th>
                  <th className="text-left px-3 py-3 font-semibold text-muted-foreground uppercase tracking-wider w-24">Coordenador</th>
                  {MONTHS.map(m => (
                    <th key={m.key} className="text-right px-2 py-3 font-semibold text-muted-foreground uppercase tracking-wider w-20">{m.label}</th>
                  ))}
                  <th className="text-right px-3 py-3 font-semibold text-muted-foreground uppercase tracking-wider w-24 text-primary">Total</th>
                  <th className="text-right px-3 py-3 font-semibold text-muted-foreground uppercase tracking-wider w-20">Ações</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={17} className="text-center py-12 text-muted-foreground">Carregando...</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={17} className="text-center py-12 text-muted-foreground">Nenhum registro encontrado</td></tr>
                ) : filtered.map((r: any, i: number) => {
                  const rowTotal = MONTHS.reduce((s, m) => s + ((r as any)[m.key] || 0), 0);
                  return (
                      ...
                      {MONTHS.map(m => (
                        <td key={m.key} className="px-2 py-2 text-right">
                          <EditableCell
                            ...
                            type="number"
                            formatter={v => v !== null && v !== undefined && v !== "" ? formatCurrency(Number(v)) : "-"}
              ...
              {filtered.length > 0 && (
                <tfoot>
                  <tr className="border-t-2 border-primary/30 bg-primary/5">
                    ...
                    {MONTHS.map(m => (
                      <td key={m.key} className="px-2 py-3 text-right font-semibold text-xs text-foreground">
                        {formatCurrency(columnTotals[m.key] || 0)}
                      </td>
                    ))}
                    <td className="px-3 py-3 text-right font-bold text-primary text-xs">
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle>Nova Filial - Faturamento {ano}</DialogTitle></DialogHeader>
      ...
      <Dialog open={deletingId !== null} onOpenChange={() => setDeletingId(null)}>
        <DialogContent className="bg-card border-border">
          ...
          <p className="text-sm text-muted-foreground">Deseja remover este registro de faturamento?</p>
          <DialogFooter>
